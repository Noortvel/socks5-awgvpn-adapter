...
(
    while true; do
        ...
        # Проверка 3: работает ли SOCKS5 прокси (TCP connect)
...

{
    ...
    echo "#   DNS=8 — DNS через обфусцированный VPN может быть медленным"
    ...
    echo "timeouts 2 10 60 120 300 3600 8 60 30 5"
    echo ""
    echo "# Лимит одновременных соединений"
    echo "maxconn 500"
    echo ""
    echo "# MSS clamping для VPN-туннеля (MTU 1420 - 40 IP+TCP = 1380)"
    ...
    echo "socks -p$SOCKS_PORT -ocTCP_NODELAY,SO_KEEPALIVE -osTCP_NODELAY,SO_KEEPALIVE"
} > "$PROXY_CONF"
